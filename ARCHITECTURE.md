...
| **authenticate middleware not wired onto group/expense routes** | The `authenticate` and `requireGroupMembership` middlewares are now applied to all group-scoped `:id` routes, EXCEPT the two DELETE routes (`/expenses/:id` and `/groups/settlements/:id`), which remain open pending R6b. |
