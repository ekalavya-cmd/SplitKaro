...
| ✅ Delete settlement                            | DELETE `/api/groups/settlements/:id` — Deletes settlement row; balances auto-recalculate                                                                                                       |
...
## §5 Known Architectural Assumptions

These are explicit assumptions baked into the current implementation that will need to be re-verified if the listed future features are ever added.

| Assumption                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Where it lives                                                                                                                                   | What breaks if violated                                                                                                                                                                                                   | Trigger condition                                            |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| **Group membership can only GROW, never shrink.** The equal-distribution defaults for non-matching tabs in the expense edit modal are computed from `group.members` (current membership) rather than `initialData.splits` (historical per-expense split rows). This is correct because members can join a group after an expense was created (via `POST /api/groups/invite/:token/join`), making `initialData.splits` potentially undercounting. The code assumes it can never OVERcount — i.e., every historical split row still maps to a real, current group member. | [`AddExpenseModal.jsx`](file:///d:/splitKaro/frontend/src/components/AddExpenseModal.jsx) — `equalDefaultExact` / `equalDefaultPercentage` block | A removed member could appear in `initialData.splits` for old expenses but be absent from `group.members`, causing the equal-default computation to omit them silently — wrong member count and wrong per-member amounts. | Adding a **"remove member from group"** endpoint or UI flow. |
